#!/usr/bin/env python3
"""Agent acts on the clinic room. Every action is traced by the rust runtime."""
...
class Room:
    ...
    def door(self) -> str:
        ...
        return (
            ...
            f"Time: {self.v.get('duration_min', 8)} min\n"
    ...
    def ask(self, item_id: str) -> dict[str, Any]:
        ...
        if item_id in hidden:
            ...
            revealed = False
        self.tracer.append(
            {"kind": "ask", "item": item_id, "ok": True, "revealed": revealed}
        )
        return {"ok": True, "item": item_id, "finding": finding, "revealed": revealed}

    def examine(self) -> dict[str, Any]:
        finding = (self.v.get("hidden") or {}).get("examine")
        self.tracer.append({"kind": "examine", "ok": bool(finding)})
        if not finding:
            return {"ok": True, "finding": "No extra exam findings beyond the chart vitals."}
    ...
    def order(self, test_id: str) -> dict[str, Any]:
        ...
        permitted = test_id in allowed or not allowed
        self.tracer.append({"kind": "order", "test": test_id, "permitted": permitted})
        if test_id in allowed:
            return {"ok": True, "test": test_id, "result": "Ordered. Result pending — disposition first."}
        return {
            "ok": True,
            "test": test_id,
            "result": "Logged. The examiner will decide if this was indicated.",
        }
    ...
    def submit(self, score_dir: Path) -> dict[str, Any]:
        ...
        result = score_trace(copied, self.v["id"])
        write_score(result, score_dir / "score.json")
        return result
